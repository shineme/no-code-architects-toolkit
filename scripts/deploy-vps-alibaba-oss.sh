...
# ============================================
# No-Code Architects Toolkit - 阿里云OSS一键部署脚本
# ============================================
# 
# 使用方法:
...
# 收集阿里云OSS配置
echo ""
print_info "请提供阿里云OSS配置信息"
echo "提示: 可在阿里云控制台获取这些信息"
echo "  - Bucket: https://oss.console.aliyun.com/"
echo "  - AccessKey: https://ram.console.aliyun.com/users"
echo ""

# 地域选择
echo "请选择阿里云OSS地域:"
echo "1) 华东1(杭州) - oss-cn-hangzhou"
echo "2) 华东2(上海) - oss-cn-shanghai"
echo "3) 华北1(青岛) - oss-cn-qingdao"
echo "4) 华北2(北京) - oss-cn-beijing"
echo "5) 华北3(张家口) - oss-cn-zhangjiakou"
echo "6) 华南1(深圳) - oss-cn-shenzhen"
echo "7) 西南1(成都) - oss-cn-chengdu"
echo "8) 中国(香港) - oss-cn-hongkong"
echo "9) 新加坡 - oss-ap-southeast-1"
echo "10) 其他 (手动输入)"
read -p "请选择地域 (1-10): " region_choice

case $region_choice in
    1) S3_REGION="oss-cn-hangzhou" ;;
    2) S3_REGION="oss-cn-shanghai" ;;
    3) S3_REGION="oss-cn-qingdao" ;;
    4) S3_REGION="oss-cn-beijing" ;;
    5) S3_REGION="oss-cn-zhangjiakou" ;;
    6) S3_REGION="oss-cn-shenzhen" ;;
    7) S3_REGION="oss-cn-chengdu" ;;
    8) S3_REGION="oss-cn-hongkong" ;;
    9) S3_REGION="oss-ap-southeast-1" ;;
    10) 
        read -p "请输入地域代码 (例: oss-cn-hangzhou): " S3_REGION
        ;;
    *) 
        print_error "无效选择"
        exit 1
...
S3_ENDPOINT_URL="https://${S3_REGION}.aliyuncs.com"

echo ""
read -p "请输入阿里云AccessKey ID: " S3_ACCESS_KEY
read -p "请输入阿里云AccessKey Secret: " S3_SECRET_KEY
read -p "请输入Bucket名称: " S3_BUCKET_NAME

# 询问是否使用内网endpoint
echo ""
read -p "VPS是否与OSS在同一地域? (使用内网可节省流量费) (y/n): " use_internal
if [[ $use_internal =~ ^[Yy]$ ]]; then
    S3_ENDPOINT_URL="https://${S3_REGION}-internal.aliyuncs.com"
